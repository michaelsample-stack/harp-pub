...
## 9 September 2026 — Estimating what the services could not answer

**Michael, for a time-sensitive deliverable.** Features the detection service
and the species rasters return nothing for have their values estimated from
the nearest features that do have one. About 7% of a month for dates and under
1% for species.

**On by default and part of the pipeline**, not an option. A deliverable with
blank fields is not a deliverable.
...
**Above 15% the run says so loudly and the stage goes amber.** Not a rule
about what is acceptable - the point at which somebody should look, because a
month where one feature in six was estimated is one where something upstream
did not answer.

**Every filled feature says so twice**, on `harp_estimated` and in the same
basis field the real values use. Every other feature carries a basis naming
where its value came from; an estimate that looked like an observation would
be the only one that did not, and this is a regulatory deliverable.
`Estimated` is carried into the delivered view so a file containing estimates
can be told from one that does not.

---

## 9 September 2026 — The reference data ships with the package

**Michael.** The supplier register, alias table, mill locations and stated
areas moved from `data/registry` into `harp/reference/`, and a run falls back
to them when nothing else is given.

**Because selecting them is itself a failure mode.** A run without mill
locations builds a circle round a town name for every supplier and says
nothing. A run without the register builds no search areas at all. Both
happened, and neither announced itself.

**A local copy still wins**, so a file somebody is editing keeps being used.
The packaged copy is the floor, not an override.

**And a register that asks for nothing is now flagged.** A report *about* the
suppliers reads perfectly well as a register - right sheet, right columns,
eighty-five rows - and reports that nobody needs a search area. That is either
a finished month or the wrong file, and the run should not have to reach stage
three to find out.

---

## 8 September 2026 — Producer name on every search area

**Found in a run log.** `ProducerName` was omitted on 3,582 of 3,781 features
- 95% of the month. Only the operator-tenure route set it on a search area;
district, county, national forest, stated area and mill buffer all built areas
without one, so every detection inside them inherited a blank.

The delivered EUDR file would have named a producer on 5% of it.

**A search area now names the supplier the client bought from**, with
`harp_producer_source` saying that is what it is - the supplier is not always
the harvester, and a broker names somebody else on the register.

**A bare supplier code is still never used as a name.** That rule came from
`WWW` and it holds here.

---

## 8 September 2026 — Harvest dates are bracketed, and say so

**The client, on behalf of their downstream customers.** Every feature carries
`HarvestStartDate` and `HarvestEndDate`, one month either side of when a
satellite observed the change.

**The field names are what was asked for.** They read as facts and they are
derived, so `harp_harvest_basis` carries the qualification on every feature
and the raw observation stays on `harp_detected_first`.

**A reversed pair is withheld, not swapped.** Swapping asserts an order the
producer did not state; shipping puts an end date before a start date in front
of a regulator.

**And P1a needed its own detection call**, since it resolves without one. The
blocks are buffered, dissolved into a single shape and submitted once.

---

## 8 September 2026 — Species from national rasters, not the provincial one

**Michael, after the client asked for species per harvest area.** Canada's
annual leading-species raster and the US forest type raster, read through
Earth Engine.

**The BC vegetation inventory was tried first and abandoned.** It works, and
where it has attributes it is finer - a surveyed stand boundary with a
percentage per species beats a pixel count. But 48 of 171 harvest polygons
overlapped inventory carrying no species at all: they were detections of
recent harvest, and the inventory had already cleared those polygons because
it knew the ground had been cut. The one thing we want to know is the thing it
blanks.

**The two rasters are not like for like**, and the design document says so.
Canada classifies the leading species per pixel; TreeMap has no species band
and carries the forest type, which is named for its leading species but
classifies the stand. The percentages should not be averaged across the
border.

**Class names come from the assets themselves** wherever they publish them.
Two earlier tables were transcribed from documentation and both were wrong -
one had Douglas-fir as class 1 where the published table has amabilis fir.
...
| 8 Sep | `is_us` read `harp_jurisdiction` and `ProducerCountry` as one field | They disagree about CA - California in the first, Canada in the second - so a feature with no jurisdiction went to the wrong raster. |
| 8 Sep | A service failure on one source ended the whole run | It died at source 175 of 221 and lost the 174 that had resolved. A source that raises is now recorded as unresolved and the run carries on. |
| 8 Sep | Search areas were built with no jurisdiction | `catchments.py` never set `harp_jurisdiction` on any of its seven routes, and every detection inherits from a search area. The EUDR country field derives from it and a blank falls back to the config default — so Washington and Californian harvest was being declared as Canadian. Found while chasing 24 features that got no species: they were in California and had been sent to the Canadian raster. |
